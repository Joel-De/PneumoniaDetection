...
class PneumoniaDetectionDataset(Dataset):
    def __init__(
        self, datasetInformationDir: Union[str, Path], imgSize=224, transform=None
    ):
        """
    ...
    def getLungClassMap() -> Dict:
        ...
        :return: Returns dictionary of index -> ID
        """
        return {"No Lung Opacity / Not Normal": 0, "Normal": 1, "Lung Opacity": 2}
    ...
    def __len__(self) -> int:
        return (
            min(self.maxDatasetLen, len(self.datasetInformation))
            if self.maxDatasetLen
            else len(self.datasetInformation)
        )
    ...
    def __getitem__(self, idx: int):
        ...
        """
        img = Image.open(
            os.path.join(self.rootDir, self.datasetInformation[idx]["FilePath"])
        ).convert("RGB")
        if self.transform:
        ...
        lungClass = [0, 0, 0]
        lungClass[
            self.getLungClassMap()[
                self.datasetInformation[idx]["AdditionalInformation"]
            ]
        ] = 1
...
if __name__ == "__main__":
    dat = PneumoniaDetectionDataset(
        r"F:\PycharmProjects\PneumoniaDetection\RSNADataset\Train\datasetInformation.json"
    )
    print(dat.__len__())
